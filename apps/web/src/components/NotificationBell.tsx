...
export default function NotificationBell() {
  ...
  // Hook up Socket.IO Client for realtime events
  useEffect(() => {
    const token = typeof window !== 'undefined' ? localStorage.getItem('accessToken') : null;
    if (!token) return;
    ...
    socket.on('connect', () => {
      console.log('🔌 Realtime notifications socket connected successfully.');
    });

    socket.on('notification', (newNotif) => {
      console.log('🔔 New realtime notification received:', newNotif);
      ...
      {isOpen && (
        <div className="absolute right-0 mt-3 w-80 bg-surface border border-border-custom rounded-2xl shadow-lg z-50 overflow-hidden py-1">
          <div className="px-4 py-3 border-b border-border-custom/50 flex justify-between items-center bg-slate-50/50">
            <span className="text-xs font-black text-primary-text uppercase tracking-wide">Recent Alerts</span>
            {unreadCount > 0 && (
              <button
          ...
          <div className="px-4 py-2 border-t border-border-custom/50 bg-slate-50/50 text-center">
            <a
              href="/notifications"
              className="text-[10px] text-primary font-black uppercase tracking-wider hover:underline block py-1"
            >
