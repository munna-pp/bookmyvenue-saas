  ...
  const handleOverrideSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        getApiUrl(`/api/v1/admin/bookings/${overridingBooking.id}/override`),
        {
          method: 'PATCH',
          headers: {
            ...getAuthHeaders(),
    ...
    <main className="min-h-screen bg-background text-primary-text flex flex-col justify-between">
      ...
      <header className="border-b border-border-custom bg-surface py-4 px-6 md:px-12 flex justify-between items-center shadow-xs">
        ...
        <nav className="flex items-center gap-4">
          ...
          <a href="/admin/bookings" className="text-primary transition font-bold text-xs">
            Bookings Queue
          </a>
          <button
      ...
      <section className="flex-1 max-w-7xl mx-auto w-full px-6 md:px-12 py-10 flex flex-col gap-8">
        <div>
          <h1 className="text-3xl font-extrabold text-primary-text tracking-tight">
            System Bookings Audit Queue
          </h1>
          <p className="text-sm text-body-text">
            Monitor all system reservations, override status states, and resolve host-client dispute
            cases.
          </p>
        </div>
          ...
          {loading ? (
            ...
            <div className="p-20 text-center flex flex-col items-center gap-3">
              ...
              <p className="text-xs text-body-text max-w-sm">
                No venue reservations or booking records exist in system memory.
              </p>
            </div>
          ) : (
      ...
      {overridingBooking && (
        <div className="fixed inset-0 bg-primary-text/40 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <div className="bg-surface border border-border-custom w-full max-w-md rounded-3xl p-6 md:p-8 shadow-lg flex flex-col gap-5">
            <div>
              <h3 className="text-lg font-bold text-primary-text flex items-center gap-1.5">
                <Wrench className="text-primary" /> Override Dispute Status
              </h3>
              <p className="text-xs text-body-text mt-1">
                Force system state transition updates for booking #
                {(overridingBooking as any).bookingNumber}. These overrides bypass the standard
