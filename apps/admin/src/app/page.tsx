      ...
      <aside className="w-64 bg-surface border-r border-border-custom flex flex-col justify-between p-6">
        <div className="flex flex-col gap-8">
          ...
          <nav className="flex flex-col gap-2">
            <a
              href="#"
              className="bg-primary/10 text-primary px-4 py-2.5 rounded-xl font-bold text-sm"
            >
              Dashboard
            </a>
            <a
              href="#"
              className="text-secondary-text hover:bg-card-bg px-4 py-2.5 rounded-xl text-sm transition"
            >
              Manage Users
            </a>
            <a
        ...
        <header className="flex justify-between items-center mb-8">
          ...
          <div className="flex items-center gap-6">
            <span className="text-xs font-semibold text-muted-text">
              Server Status: <span className="text-green-600 font-bold">● Healthy</span>
            </span>
            <NotificationBell />
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          {[
            ...
            { label: 'Listed Venues', value: '382', change: '+15.4%', color: 'border-l-accent' },
            {
              ...
              color: 'border-l-pending',
            },
          ].map((stat, i) => (
            <div
              key={i}
              className={`bg-card-bg border-l-4 ${stat.color} border border-border-custom/55 rounded-2xl p-6 shadow-xs`}
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-surface border border-border-custom rounded-2xl p-6">
            <h2 className="text-lg font-bold mb-4 text-primary-text flex justify-between items-center">
              ...
              <span className="bg-pending/10 text-pending text-xs px-2 py-0.5 rounded-full">
                14 Request(s)
              </span>
            </h2>
            <div className="flex flex-col gap-4">
              {[
                ...
                <div
                  key={i}
                  className="flex justify-between items-center p-4 border border-border-custom/40 rounded-xl bg-card-bg"
                >
                  <div>
                    <span className="block font-bold text-sm text-primary-text">{venue.name}</span>
                    <span className="block text-xs text-body-text">
                      {venue.type} • Capacity: {venue.capacity} • Owner: {venue.owner}
                    </span>
                  </div>
                  <div className="flex gap-2">
                    <button className="bg-primary text-surface text-xs px-3 py-1.5 rounded-lg hover:bg-primary/95 font-bold cursor-pointer">
                      Approve
                    </button>
                    <button className="bg-transparent border border-border-custom text-secondary-text text-xs px-3 py-1.5 rounded-lg hover:bg-background font-bold cursor-pointer">
                      Reject
                    </button>
                  </div>
                </div>
          ...
          <div className="bg-surface border border-border-custom rounded-2xl p-6">
            ...
            <div className="flex flex-col gap-4 font-mono text-[11px] text-body-text">
              {[
                {
                  time: '2026-06-26 22:15:02',
                  user: 'SYSTEM',
                  msg: 'Backup completed successfully.',
                },
                {
                  time: '2026-06-26 22:12:11',
                  user: 'admin_joseph',
                  msg: 'Blocked user ID #9103 due to terms violation.',
                },
                {
                  time: '2026-06-26 22:04:45',
                  user: 'owner_ramesh',
                  msg: 'Submitted new venue "Grand Ballroom & Garden" for approval.',
                },
              ].map((log, i) => (
