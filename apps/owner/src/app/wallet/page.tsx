    ...
    <main className="min-h-screen bg-background text-primary-text flex flex-col justify-between">
      ...
      <header className="border-b border-border-custom bg-surface py-4 px-6 md:px-12 flex justify-between items-center shadow-xs">
        ...
        <nav className="flex items-center gap-6">
          ...
          <a href="/owner/wallet" className="text-primary transition font-bold text-xs">
            Wallet
          </a>
        </nav>
      </header>
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-extrabold text-primary-text tracking-tight">
              Host Earnings Wallet
            </h1>
            <p className="text-sm text-body-text">
              Track your net payouts, active ledger credits, and withdrawal transactions.
            </p>
          </div>
          <button
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            ...
            <div className="bg-surface border border-border-custom rounded-3xl p-6 md:p-8 shadow-md flex flex-col gap-6">
              ...
              <div>
                ...
                <span className="block text-[10px] text-muted-text mt-1.5 font-medium">
                  Payout value eligible for withdrawal
                </span>
              </div>
              <button
                onClick={() =>
                  alert(
                    'Bank payouts are initiated automatically twice a week. Direct manual withdrawal is currently locked.'
                  )
                }
              ...
              {ledger.length === 0 ? (
                <div className="p-20 text-center flex flex-col items-center gap-3 bg-surface border border-border-custom rounded-3xl">
                  ...
                  <p className="text-xs text-body-text max-w-sm">
                    Payments credited upon booking verification will be logged here.
                  </p>
                </div>
              ) : (
                <div className="bg-surface border border-border-custom rounded-3xl overflow-hidden shadow-xs">
                  <div className="divide-y divide-border-custom/10">
                    {ledger.map((tx) => (
                      <div
                        key={tx.id}
                        className="p-5 flex justify-between items-center hover:bg-card-bg/25 transition"
                      >
                        <div className="flex gap-3 items-center">
                          <div
                            className={`p-2 rounded-xl ${tx.type === 'CREDIT' ? 'bg-green-500/10 text-green-600' : 'bg-red-500/10 text-red-600'}`}
                          >
                            {tx.type === 'CREDIT' ? (
                          ...
                          <div>
                            <span className="font-extrabold text-sm text-primary-text block">
                              {tx.description}
                            </span>
                        ...
                        <div className="text-right">
                          <span
                            className={`font-black text-sm block ${tx.type === 'CREDIT' ? 'text-green-600' : 'text-red-600'}`}
                          >
                            {tx.type === 'CREDIT' ? '+' : '-'}₹{tx.amount.toLocaleString('en-IN')}
                          </span>
                          <span
                            className={`inline-block mt-1 px-2 py-0.5 rounded-full text-[8px] font-bold uppercase tracking-wider ${getTxTypeBadgeClass(tx.type)}`}
                          >
                            {tx.type}
                          </span>
                        </div>
