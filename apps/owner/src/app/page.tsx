    ...
    <main className="min-h-screen bg-background text-primary-text flex flex-col justify-between">
      ...
      <header className="border-b border-border-custom bg-surface py-4 px-6 md:px-12 flex justify-between items-center shadow-xs">
        ...
        <nav className="flex items-center gap-6">
          <a href="/" className="text-secondary-text hover:text-primary transition font-medium text-sm">Switch to Guest Mode</a>
      ...
      <section className="flex-1 max-w-7xl mx-auto w-full px-6 md:px-12 py-10 flex flex-col gap-8">
        <div>
          <h1 className="text-3xl font-extrabold text-primary-text tracking-tight">Host Dashboard</h1>
          <p className="text-sm text-body-text">Manage your venues, availability, bookings, and monitor your earnings.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            ...
            <div key={i} className="bg-surface border border-border-custom rounded-2xl p-6 shadow-xs">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 bg-surface border border-border-custom rounded-2xl p-6 flex flex-col gap-4">
            ...
            {[
              { guest: 'Amit Kumar', venue: 'Grand Ballroom', date: 'Oct 14, 2026', price: '₹45,000' },
              { guest: 'Sarah Wilson', venue: 'Skyline Meeting Room', date: 'Jul 04, 2026', price: '₹8,500' },
            ].map((req, i) => (
              <div key={i} className="border border-border-custom/40 bg-card-bg p-4 rounded-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                  <span className="block font-bold text-sm text-primary-text">{req.guest}</span>
                  <span className="block text-xs text-body-text">{req.venue} • {req.date}</span>
                  <span className="block text-xs font-bold text-primary mt-1">{req.price}</span>
                </div>
                <div className="flex gap-2 w-full md:w-auto">
                  <button className="flex-1 md:flex-initial bg-primary text-surface text-xs px-4 py-2 rounded-lg font-bold hover:bg-primary/95 cursor-pointer">Accept</button>
                  <button className="flex-1 md:flex-initial bg-transparent border border-border-custom text-secondary-text text-xs px-4 py-2 rounded-lg font-bold hover:bg-background cursor-pointer">Reject</button>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="bg-surface border border-border-custom rounded-2xl p-6 flex flex-col gap-4">
            ...
            {[
              { name: 'Grand Ballroom', type: 'Wedding Hall', status: 'Approved', statusColor: 'bg-green-100 text-green-800' },
              { name: 'Skyline Meeting Room', type: 'Meeting Room', status: 'Approved', statusColor: 'bg-green-100 text-green-800' },
              { name: 'Rustic Farmhouse', type: 'Farm House', status: 'Pending Review', statusColor: 'bg-amber-100 text-amber-800' },
            ].map((venue, i) => (
              <div key={i} className="flex justify-between items-center py-2 border-b border-border-custom/20 last:border-0">
                <div>
                ...
                <span className={`text-[9px] px-2 py-0.5 rounded-full font-bold ${venue.statusColor}`}>{venue.status}</span>
              </div>
            ))}
          </div>
        </div>
