...
export const createReviewSchema = z.object({
  ...
  review: z.string().min(10, 'Review must be at least 10 characters long').max(2000, 'Review cannot exceed 2000 characters'),
  images: z.array(z.string().url('Image must be a valid URL')).max(5, 'Maximum of 5 images allowed').optional(),
});
...
export const ownerReplySchema = z.object({
  reply: z.string().min(2, 'Reply must be at least 2 characters long').max(1000, 'Reply cannot exceed 1000 characters'),
});
