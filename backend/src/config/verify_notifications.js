...
async function runNotificationTests() {
  ...
  try {
    ...
    const conn = await mongoose.createConnection(`${mongoUri}/bookmyvenue_notifications`).asPromise();
    await conn.collection('notifications').deleteMany({ userId: new mongoose.Types.ObjectId(customerId) });
    await conn.close();
  ...
  try {
    ...
    await conn.collection('bookings').updateOne(
    ...
    const hmac = crypto.createHmac('sha256', process.env.RAZORPAY_KEY_SECRET || 'your-razorpay-key-secret');
  ...
  console.log(`Total realtime socket notifications received: ${receivedSocketNotifications.length}`);
  ...
  try {
    ...
    console.log(`✅ GET /notifications returned ${notifications.length} items (Page: ${listJson.data.page}/${listJson.data.pages}).`);
    if (notifications.length === 0) throw new Error('No in-app notifications persisted in MongoDB.');
    ...
    console.log(`✅ PATCH /notifications/:id/read verified. Status read: ${readJson.data.notification.read}`);
    ...
    if (unreadVerifyJson.data.count !== 0) throw new Error('Unread count is not zero after read-all.');
    ...
    const isDeletedPresent = list2Json.data.notifications.some(n => (n._id === notificationId || n.id === notificationId));
    console.log(`✅ Verified soft deleted item is absent in lists: ${!isDeletedPresent ? 'YES' : 'NO'}`);
    if (isDeletedPresent) throw new Error('Soft-deleted notification still returned in fetch list.');
